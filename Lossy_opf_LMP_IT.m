function [MVAbase, bus, gen, gencost, branch, f, success, et] = Lossy_opf_LMP_IT(c, mpc, Va0, Vm0, Pbus0, Ploss0, Qbus0, Qloss0,varargin)
...
%% A 节点功率平衡等式约束
Constraints = [Constraints; (Pbus == Cg * Pg - Pd - Pequ):'Balance_P'];
Constraints = [Constraints; (Qbus == Cg * Qg - Qd - Qequ):'Balance_Q'];
% Constraints = [Constraints; sum(Cg * Pg - Pd) == sum(Pequ)];   % nodal real power balance (MW)
% Constraints = [Constraints; sum(Cg * Qg - Qd) - sum(Qequ) == 0];   % nodal real power balance (MW)
...
Constraints=[Constraints; Pf == (GSF.GSF_PP_F * Pbus + GSF.GSF_PQ_F * Qbus)+ 0.5 * Ploss];
Constraints=[Constraints; Qf == (GSF.GSF_QP_F * Pbus + GSF.GSF_QQ_F * Qbus)+ 0.5 * Qloss];
Constraints=[Constraints; Pt == (GSF.GSF_PP_T * Pbus + GSF.GSF_PQ_T * Qbus)+ 0.5 * Ploss];
Constraints=[Constraints; Qt == (GSF.GSF_QP_T * Pbus + GSF.GSF_QQ_T * Qbus)+ 0.5 * Qloss];
...
%% 网损约束
if (c==3)
    Constraints=[Constraints;(Pequ == 0.25 * (Cf+Ct)' * (Ploss)):'FND_P'];
    Constraints=[Constraints;(Qequ == 0.25 * (Cf+Ct)' * (Qloss)):'FND_Q'];
else
    Constraints=[Constraints;(Pequ == 0.5 * (Cf+Ct)' * (Ploss)):'FND_P'];
    Constraints=[Constraints;(Qequ == 0.5 * (Cf+Ct)' * (Qloss)):'FND_Q'];
end
...
% LF_PVm_2 = ((Vm0(f,:).^2 - Vm0(t,:).^2) .* real(Ysc));
% LF_QVa = (Va0(f,:) - Va0(t,:)) .* imag(Ysc);
% LF_QVm_2 = ((Vm0(f,:).^2 - Vm0(t,:).^2) .* imag(Ysc));
...
LF_PVa = 2*(Va0(f,:) - Va0(t,:)) .* real(Ysc);
LF_PVm_2 = 2* ((Vm0(f,:) - Vm0(t,:))./(Vm0(f,:) + Vm0(t,:))) .* real(Ysc);
offset_P = -((Va0(f,:) - Va0(t,:)).^2 + (Vm0(f,:) - Vm0(t,:)).^2) .* real(Ysc);
LF_QVa = 2*(Va0(f,:) - Va0(t,:)) .* imag(Ysc);
LF_QVm_2 =  2* ((Vm0(f,:) - Vm0(t,:))./(Vm0(f,:) + Vm0(t,:))) .* imag(Ysc);
offset_Q =  ((Va0(f,:) - Va0(t,:)).^2 + (Vm0(f,:) - Vm0(t,:)).^2) .* imag(Ysc);
Constraints=[Constraints;(Ploss == LF_PVa .* (Va(f,:) - Va(t,:)) + LF_PVm_2 .* (Vm_2(f,:) - Vm_2(t,:)) + offset_P):'Ploss'];%LF_P_Q' * Pbus0 * Qbus0 =  P_offset
Constraints=[Constraints;(Qloss == LF_QVa .* (Va(f,:) - Va(t,:)) + LF_QVm_2 .* (Vm_2(f,:) - Vm_2(t,:)) + offset_Q):'Qloss'];%LF_Q_P' * Qbus0 * Pbus0 =  Q_offset

% Constraints=[Constraints;(Ploss == real(Ysc) .* (0.25 * ((Vm0(f,:).^2 - Vm0(t,:).^2).^2+ 2*(Vm0(f,:).^2 - Vm0(t,:).^2).*((Vm_2(f,:)-Vm0(f,:).^2)+(Vm0(t,:).^2 - Vm_2(t,:)))) ...
%     + ((Va0(f,:).^2 - Va0(t,:).^2).^2+ 2*(Va0(f,:) - Va0(t,:)).*((Va(f,:)-Va0(f,:))+(Va0(t,:) - Va(t,:)))))):'Ploss'];%LF_P_Q' * Pbus0 * Qbus0 =  P_offset
% Constraints=[Constraints;(Qloss == imag(Ysc) .* (0.25 * ((Vm0(f,:).^2 - Vm0(t,:).^2).^2+ 2*(Vm0(f,:).^2 - Vm0(t,:).^2).*((Vm_2(f,:)-Vm0(f,:).^2)+(Vm0(t,:).^2 - Vm_2(t,:)))) ...
%     + ((Va0(f,:).^2 - Va0(t,:).^2).^2+ 2*(Va0(f,:) - Va0(t,:)).*((Va(f,:)-Va0(f,:))+(Va0(t,:) - Va(t,:)))))):'Qloss'];%LF_Q_P' * Qbus0 * Pbus0 =  Q_offset
...
res.lmp_FND_P  = dual(Constraints('FND_P'))/ baseMVA;
...
% value(0.5 * (Cf+Ct)' * (0.5 * LF_PP * Pbus)).*res.lmp_FND_P
% p_branch = (res.Pf - res.Pt)/2/ mpc.baseMVA;
% r = mpc.branch(:,3);
% T = makePTDF(mpc);
%         LF = 2*(r.*p_branch)'*T;
%         LF = LF';
%       PLoss_Price = LF .* res.lmp_energy_P;%
% LMP：V
...
LMP.PfCon_Price = Cf' * (value(GSF.GSF_PP_F.^2 * Pbus + 2 * GSF.GSF_PP_F .* GSF.GSF_PQ_F * Qbus + GSF.GSF_QP_F.^2 * Pbus + 2 * GSF.GSF_QP_F .* GSF.GSF_QQ_F * Qbus) .* res.lmp_mu_SF);
LMP.PtCon_Price = Ct' * (value(GSF.GSF_PP_T.^2 * Pbus + 2 * GSF.GSF_PP_T .* GSF.GSF_PQ_T * Qbus + GSF.GSF_QP_T.^2 * Pbus + 2 * GSF.GSF_QP_T .* GSF.GSF_QQ_T * Qbus) .* res.lmp_mu_ST);
LMP.QfCon_Price = Cf' * (value(GSF.GSF_PQ_F.^2 * Qbus + 2 * GSF.GSF_PP_F .* GSF.GSF_PQ_F * Pbus + GSF.GSF_QQ_F.^2 * Qbus + 2 * GSF.GSF_QP_F .* GSF.GSF_QQ_F * Pbus) .* res.lmp_mu_SF);
LMP.QtCon_Price = Ct' * (value(GSF.GSF_PQ_T.^2 * Qbus + 2 * GSF.GSF_PP_T .* GSF.GSF_PQ_T * Pbus + GSF.GSF_QQ_T.^2 * Qbus + 2 * GSF.GSF_QP_T .* GSF.GSF_QQ_T * Pbus) .* res.lmp_mu_ST);
