function [MVAbase, bus, gen, gencost, branch, f, success, et] = Lossy_SOCP_opf_LMP(mpc, varargin)
...
obj = C2' * (Pg.^2) + C1' * Pg + ones(ng, 1)' * C0 + ...
    + C1' * (Pres_d+Pres_u);    % + C2' * (Qg.^2) + C1' * Qg + ones(ng, 1)' * C0

%% 约束条件
Constraints = [];

%% A 节点功率平衡等式约束
Constraints = [Constraints; (Pbus + Pequ + Pd  - Cg * Pg == 0):'Balance_P'];
Constraints = [Constraints; (Qbus + Qequ + Qd  - Cg * Qg == 0):'Balance_Q'];
...
Constraints=[Constraints; Qf  == -(Vm_2(f)-Vm_2(t)).* imag(Ysc)/ 2 - (Va(f)-Va(t)).*real(Ysc) + 0.5*Ploss ];
Constraints=[Constraints; Pt  == (Vm_2(t)-Vm_2(f)).* real(Ysc)/ 2 - (Va(t)-Va(f)).*imag(Ysc) + 0.5*Qloss ];
...
%% 网损约束
Constraints=[Constraints;Pequ == (Cf+Ct)' * (0.5 * Ploss)];
Constraints=[Constraints;Qequ == (Cf+Ct)' * (0.5 * Qloss)];
...
% Qf_abs = sdpvar(nl,1);
% % 原始约束的凸松弛 
% Constraints = [Constraints, 0 <= Ploss, Ploss <= Pf_abs]; 
% Constraints = [Constraints, 0 <= Qloss, Qloss <= Qf_abs]; 
% % 线性化绝对值 
...
% % 爬坡约束
% if (const.nbus_ac <= const.nline_ac && h > 1)
% Constraints = [Constraints;  (-0.1 * real(const.ac_Sgmax) <= var.Pgac - const.ac_Pg_t_1):'P_ramp_min'];
% Constraints = [Constraints;  (var.Pgac - const.ac_Pg_t_1 <= 0.1 * real(const.ac_Sgmax)):'P_ramp_max'];
% Constraints = [Constraints;  (-0.1 * imag(const.ac_Sgmax) <= var.Qgac - const.ac_Qg_t_1):'Q_ramp_min'];
% Constraints = [Constraints;  (var.Qgac - const.ac_Qg_t_1 <= 0.1 * imag(const.ac_Sgmax)):'Q_ramp_max'];
% end    
...
for i = 1:size(Pf,1)
     Constraints = [Constraints,  (-Fmax(i) <= (GSF_PP_F(i,:) * Pbus + GSF_PQ_F(i,:) * Qbus)):'Con_PF_low'];                  % branch flow limit at "from" end (p.u.)
     Constraints = [Constraints,  ((GSF_PP_F(i,:) * Pbus + GSF_PQ_F(i,:) * Qbus) <= Fmax(i)):'Con_PF_high'];                  % branch flow limit at "from" end (p.u.)
     Constraints = [Constraints,  (-Fmax(i) <= (GSF_QP_F(i,:) * Pbus + GSF_QQ_F(i,:) * Qbus)):'Con_QF_low'];
     Constraints = [Constraints,  ((GSF_QP_F(i,:) * Pbus + GSF_QQ_F(i,:) * Qbus) <= Fmax(i)):'Con_QF_high'];
      
     Constraints = [Constraints,  (Fmax(i) >= (GSF_PP_T(i,:) * Pbus + GSF_PQ_T(i,:) * Qbus)):'Con_PT_low'];                  % branch flow limit at "from" end (p.u.)
     Constraints = [Constraints,  ((GSF_PP_T(i,:) * Pbus + GSF_PQ_T(i,:) * Qbus) >= -Fmax(i)):'Con_PT_high'];                  % branch flow limit at "from" end (p.u.)
     Constraints = [Constraints,  (Fmax(i) >= (GSF_QP_T(i,:) * Pbus + GSF_QQ_T(i,:) * Qbus)):'Con_QT_low'];
     Constraints = [Constraints,  ((GSF_QP_T(i,:) * Pbus + GSF_QQ_T(i,:) * Qbus) >= -Fmax(i)):'Con_QT_high'];     
end
%  for i=1:t_poly
...
% opt.gurobi.BarConvTol = 1e-6;%barrier算法的收敛容差
% opt.gurobi.BarQCPConvTol = 1e-6;
% opt.gurobi.FeasibilityTol = 1e-6;%原始可行性容差
% opt.gurobi.MIPGap = 1e-6;
...
% LMP：Loss
% res.lmp_loss_P = dual(Constraints('Ploss'))/ baseMVA;   % lmp($/MWh)
% res.lmp_loss_Q = dual(Constraints('Qloss'))/ baseMVA;   % lmp($/MWh)
...
% LMP.PLoss_Price = (-(Cft)'* LMP.LF_P).*(-2*pinv(Cf + Ct)*res.lmp_loss_P);%-Cf' * LF_P .* res.lmp_energy_P
% LMP.QLoss_Price = (-(Cft)'* LMP.LF_Q).*(-2*pinv(Cf + Ct)*res.lmp_loss_Q);%-Cf' * LF_Q .* res.lmp_energy_Q

% LMP.SP_loss_P = 2 * pinv(Cf+Ct) * dual(Constraints('Ploss'))/ baseMVA;   % lmp($/MWh)
% LMP.SP_loss_Q = 2 * pinv(Cf+Ct) * dual(Constraints('Qloss'))/ baseMVA;   % lmp($/MWh)
% LMP.LF_P = value(LF_PP * Pbus + LF_P_Q * Qbus);%LF_P_Q' * Pbus0 * Qbus0 =  P_offset
% LMP.LF_Q = value(LF_QQ * Qbus + LF_Q_P * Pbus);%LF_Q_P' * Qbus0 * Pbus0 =  Q_offset
% % LMP.LF_P = value((C_PVm(f,:) - C_PVm(t,:)).^2 * Pbus + 2 * ((C_PVm(f,:) - C_PVm(t,:)) .* (C_QVm(f,:) - C_QVm(t,:)) * Qbus) ...
% %      + (C_PVa(f,:) - C_PVa(t,:)).^2 * Pbus + 2 * ((C_PVa(f,:) - C_PVa(t,:)) .* (C_QVa(f,:) - C_QVa(t,:)) * Qbus)) .* real(Ysc);
% % LMP.LF_Q = value((C_QVm(f,:) - C_QVm(t,:)).^2 * Qbus + 2 * ((C_PVm(f,:) - C_PVm(t,:)) .* (C_QVm(f,:) - C_QVm(t,:)) * Qbus) ...
% %     + (C_QVa(f,:) - C_QVa(t,:)).^2 * Qbus + 2 * ((C_PVa(f,:) - C_PVa(t,:)) .* (C_QVa(f,:) - C_QVa(t,:)) * Qbus)) .* imag(Ysc);
...
LMP.PLoss_Price = (-(Cft)'* LMP.LF_P).*(-2 * LMP.SP_loss_P);%-Cf' * LMP.LF_P .* res.lmp_energy_P
LMP.QLoss_Price = (-(Cft)'* LMP.LF_Q).*(-2 * LMP.SP_loss_Q);%-Cf' * LF_Q .* res.lmp_energy_Q
...
% LMP.SfCon_Price = GSF.GSF_PP_F' * res.lmp_mu_SF;
% LMP.StCon_Price = GSF.GSF_PP_F' * res.lmp_mu_ST;
...
% 发电备用价格
LMP.lmp_res_down = (dual(Constraints('Res_d_max')) - dual(Constraints('Res_d_min'))) / baseMVA;   % lmp($/MWh)
%LMP{1,1}.lmp_res_down = dual(constr_det('Res_d')) / mpcac.baseMVA;
LMP.lmp_res_up = (dual(Constraints('Res_u_max')) - dual(Constraints('Res_u_min'))) / baseMVA;   % lmp($/MWh)
LMP.lmp_res = LMP.lmp_res_up + LMP.lmp_res_down;
LMP.total_R = C1 / baseMVA + LMP.lmp_res;
